...
impl PartialEq for SpecialWorkspace {
    fn eq(&self, other: &Self) -> bool {
        self.id == other.id
    }
}

impl PartialOrd for SpecialWorkspace {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        self.id.partial_cmp(&other.id)
    }
}

impl Ord for SpecialWorkspace {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        self.id.cmp(&other.id)
    }
}

impl Eq for SpecialWorkspace {}
...
impl HyprWorkspacesWidget {
    ...
    fn start_updates(&self, config: Arc<WorkspacesConfig>) {
                ...
                loop {
                    ...
                    let result = match Workspaces::get() {
                        Ok(ws) => {
                            ...
                            let workspace_ids = workspaces.iter().map(|w| SpecialWorkspace {
                                id: w.id,
                                name: w.name.clone(),
                            });
                            let workspace_ids: Vec<SpecialWorkspace> = if show_special_workspaces {
        ...
        let mut prev_workspaces: Vec<SpecialWorkspace> = vec![];
    ...
    fn rebuild_buttons(
        container: &gtk::Box,
        workspace_ids: &[SpecialWorkspace],
        prev_active_id: i32,
        format: &str,
        ...
        let mut workspace_id_array: Vec<SpecialWorkspace> = workspace_ids.to_vec();
        for i in 1..=min_workspace_count {
            let workspace = SpecialWorkspace {
                id: i,
                ..Default::default()
            };
