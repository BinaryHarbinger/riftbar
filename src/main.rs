...
fn get_config_dir() -> PathBuf {
    let mut config_path = PathBuf::from(std::env::var("HOME").unwrap_or_else(|_| String::from("~")));
        ...
        match output {
            Ok(output) => {
                if output.status.success() {
                    ...
                    eprintln!("SCSS compilation failed: {}", String::from_utf8_lossy(&output.stderr));
            ...
            Err(e) => {
                eprintln!("Failed to run sass command: {}. Make sure 'sass' is installed.", e);
                // Fall back to CSS if it exists
                if css_path.exists() {
                    return Some(css_path);
                }
